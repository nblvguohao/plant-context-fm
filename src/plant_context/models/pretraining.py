...
def pretrain_masked_reconstruction(
    ...
    head = nn.Linear(d_model, n_features)
    optimizer = torch.optim.Adam(list(encoder.parameters()) + list(head.parameters()), lr=lr)

    encoder_input_t = torch.tensor(encoder_input, dtype=torch.float32)
    padding_mask_t = torch.tensor(padding_mask)
    targets_t = torch.tensor(features_scaled, dtype=torch.float32)
    mask_positions_t = torch.tensor(mask_positions)
